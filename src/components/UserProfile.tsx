...
import { useAdminClientProfile, type CLientProfileData } from '../hooks/useAdminClientProfile';
import { GoClock } from 'react-icons/go';
import { ImUser } from 'react-icons/im';
import { IoMdClose } from 'react-icons/io';
import { AiOutlineEye, AiOutlineEyeInvisible } from 'react-icons/ai';
            ...
            {/* {breadcrumbTrail.map((item, i) => (
              <span key={i}>
                ...
                {i === breadcrumbTrail.length - 1 ? (
                  ...
                  <button onClick={onBack} className="underline cursor-pointer">
                    {item}
                  </button>
                )}{' '}
              </span>
            ...
            <ul className="flex space-x-4">
              {tabs.map(tab => (
                <li key={tab}
                  ...
                  className={`pb-2 cursor-pointer ${activeTab===tab ? 'border-b-2 text-primary':'text-gray-500'}`}>
                  {tab}
                </li>
              ))}
            </ul>
...
{showResetPasswordModal && (
        <div className="fixed inset-0 bg-black/10 bg-opacity-20 flex items-center justify-center">
          <div className="bg-white p-6 rounded shadow-lg w-80" ref={modalRef}>
            <IoMdClose onClick={()=>setShowResetPasswordModal(false)} className="absolute top-2 right-2 cursor-pointer text-xl text-primary" />
            <h2 className="text-lg font-bold mb-4">Reset Password?</h2>
            <form className="space-y-4">
              <div className="relative">
                <label className="block mb-1">New Password</label>
                <input type={showPassword?'text':'password'} className="w-full border px-3 py-2 pr-10" />
                <button type="button" onClick={()=>setShowPassword(p=>!p)} className="absolute right-3 top-9">
                  {showPassword?<AiOutlineEyeInvisible/>:<AiOutlineEye/>}
                </button>
              </div>
              {/* confirm password  
