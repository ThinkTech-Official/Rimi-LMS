      ...
      <div className="relative" ref={profileDropdown}>
        ...
        >
          <span className="flex gap-2 items-center" title={admin?.name}>
            ...
            {admin ? admin.name.length > 10 ? `${admin.name.slice(0, 10)}...` : admin.name : "Please log in"}
          </span>
          <MdKeyboardArrowRight
            className={`h-4 w-4 2xl:w-6 2xl:h-6 transform transition ${
        ...
        {isProfileMenuOpen && (
          <div className="absolute mt-4 ml-1 w-36 2xl:w-40 rounded-sm shadow-lg bg-white border border-[#E9EEF1] z-10">
            <ul className="py-1 text-sm 2xl:text-base text-gray-700">
              <li>
                <button
              ...
              <li>
                <button
                  ...
                  className="w-full text-left px-4 py-2 hover:bg-primary hover:text-white cursor-pointer flex gap-2 items-center"
                >
                  <IoIosLogOut className="h-4 w-4 2xl:w-5 2xl:h-5" /> {t("Logout")}
                </button>
              </li>
            </ul>
          </div>
